...
Releases run through [np](https://github.com/sindresorhus/np), which handles the version bump, git tag, npm publish, and GitHub release draft in one interactive flow.
...
2. **Scope access.** The package publishes under the `@himynameisdave` scope, which your npm user (or an org you belong to) must own. Personal npm usernames own their matching scope automatically.
